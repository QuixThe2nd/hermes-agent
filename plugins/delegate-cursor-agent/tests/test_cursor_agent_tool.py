...
def test_schema_registration():
    from delegate_cursor_agent.tool import CURSOR_AGENT_SCHEMA
...
def test_check_fn_binary_found(monkeypatch):
    from delegate_cursor_agent.tool import check_cursor_agent_requirements

    monkeypatch.setattr("delegate_cursor_agent.tool.shutil.which", lambda name: "/usr/bin/agent")
...
def test_check_fn_binary_missing(monkeypatch):
    ...
    monkeypatch.setattr("delegate_cursor_agent.tool.shutil.which", lambda name: None)

    class _MissingPath(Path):
        def is_file(self):
            return False

    monkeypatch.setattr("delegate_cursor_agent.tool._local_bin_agent_path", lambda: _MissingPath("/nope/agent"))
...
def test_parse_stream_json_log():
    from delegate_cursor_agent.tool import parse_cursor_agent_log
...
def test_action_required_error_handler(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("delegate_cursor_agent.tool.resolve_cursor_agent_binary", lambda: "/usr/bin/agent")
    monkeypatch.setattr("delegate_cursor_agent.tool.subprocess.Popen", _ActionRequiredPopen)
...
def test_stall_path_terminates_process(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("delegate_cursor_agent.tool.resolve_cursor_agent_binary", lambda: "/usr/bin/agent")
    monkeypatch.setattr("delegate_cursor_agent.tool.subprocess.Popen", _StalledFakePopen)
...
def test_timeout_path_terminates_process(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("delegate_cursor_agent.tool.resolve_cursor_agent_binary", lambda: "/usr/bin/agent")
    monkeypatch.setattr("delegate_cursor_agent.tool.subprocess.Popen", _TimeoutFakePopen)
    monkeypatch.setattr("delegate_cursor_agent.tool.time.monotonic", _fake_monotonic)
...
def test_interrupt_path_terminates_process(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cursor_agent_tool, "STALL_WATCHDOG_SECONDS", 9999)
    monkeypatch.setattr(cursor_agent_tool, "_MONITOR_POLL_SECONDS", 0.001)
    monkeypatch.setattr("delegate_cursor_agent.tool._check_interrupted", lambda: True)
...
def test_non_zero_exit_reports_failure(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("delegate_cursor_agent.tool.resolve_cursor_agent_binary", lambda: "/usr/bin/agent")
    monkeypatch.setattr("delegate_cursor_agent.tool.subprocess.Popen", _NonZeroExitPopen)
...
def test_happy_path_e2e(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("delegate_cursor_agent.tool.resolve_cursor_agent_binary", lambda: "/usr/bin/agent")
    monkeypatch.setattr("delegate_cursor_agent.tool.subprocess.Popen", _StreamingFakePopen)
...
def test_handler_force_string_false(monkeypatch, tmp_path):
    from delegate_cursor_agent.tool import _handle_delegate_cursor_agent
...
def _kill_process_group_or_pid(pgid: int | None, pid: int | None) -> None:
    ...
    if pgid is not None:
        try:
            os.killpg(pgid, signal.SIGKILL)  # windows-footgun: ok — linux/macos-only plugin test
            return
        except (OSError, ProcessLookupError):
            pass
    if pid is not None:
        try:
            os.killpg(os.getpgid(pid), signal.SIGKILL)  # windows-footgun: ok — linux/macos-only plugin test
            return
        except (OSError, ProcessLookupError):
            pass
        try:
            os.kill(pid, signal.SIGKILL)  # windows-footgun: ok — linux/macos-only plugin test
        except (OSError, ProcessLookupError):
            pass
            ...
            if logs:
                ...
                if "chunk1" in logs[0].read_text(encoding="utf-8", errors="replace"):
                    ...
                    if child_pid is not None:
                        os.kill(child_pid, 0)  # windows-footgun: ok — linux/macos-only plugin test
                    break
            time.sleep(0.05)
...
def test_terminate_process_kills_sigterm_resistant_descendant(tmp_path, monkeypatch):
    ...
    from delegate_cursor_agent.tool import _terminate_process

    monkeypatch.setattr("delegate_cursor_agent.tool._TERMINATE_GRACE_SECONDS", 0.5)
...
"""
    ...
    try:
        ...
        with pytest.raises(ProcessLookupError):
            os.kill(desc_pid, 0)  # windows-footgun: ok — linux/macos-only plugin test
    finally:
        if pgid is not None:
            try:
                os.killpg(pgid, signal.SIGKILL)  # windows-footgun: ok — linux/macos-only plugin test
            ...
            try:
                os.killpg(os.getpgid(proc.pid), signal.SIGKILL)  # windows-footgun: ok — linux/macos-only plugin test
            except (OSError, ProcessLookupError):
        ...
        if desc_pid is not None:
            try:
                os.kill(desc_pid, signal.SIGKILL)  # windows-footgun: ok — linux/macos-only plugin test
            except (OSError, ProcessLookupError):
                pass
        try:
            proc.wait(timeout=1)
...
def test_child_env_guarantees_home_and_local_bin(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("delegate_cursor_agent.tool.resolve_cursor_agent_binary", lambda: "/usr/bin/agent")
    monkeypatch.setattr("delegate_cursor_agent.tool.subprocess.Popen", _EnvCapturePopen)
