...
One independent adversarial read-only review of the bounded diff file (`review-diff.txt`). The reviewer is Claude (claude-glm) via `run_agent_cli` with Read/Grep only, a Russian adversarial prompt, and at most 3–4 tool calls. Verdict must be parseable strict JSON on the final line; unparseable or unavailable = fail-closed (`review_unavailable`), never silently skipped.

**Rationale (2026 addendum):** Slice-1 originally paired Kimi K3 + Grok 4.5 (`cursor-grok-4.5-high`). The Grok pin is unresolvable on the current Cursor CLI roster. The proven supervisor-side pattern is a single Russian adversarial Claude pass over a bounded diff file (not inline diff in the prompt). `reviews.json` stores `claude_ru`. Gate: mechanical PASS AND claude verdict pass → proceed. Any `fail` with blocking findings → bounded repair (one attempt if attempts remain), then re-verify + re-review.
