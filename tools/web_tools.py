...
def _should_search_rescue_after_failure(provider) -> bool:
    """Gate one-shot keyless rescue after a search failure.

    Rescue runs when a configured ``web.search_fallbacks`` chain was present
    (rescue is the hop after chain exhaustion), or when the primary is a keyed
    ring vendor with no chain configured (the original keyless-rescue path).

    Free-tier / non-ring primaries (``brave-free``, ``ddgs``, ``searxng``, …)
    without a fallback chain surface the primary error unchanged; quota recovery
    for those backends is ``web.search_fallbacks``, not the keyless ring.
    """
    if not _rescue_eligible(provider):
        return False
    if _get_fallback_backends("search", primary=getattr(provider, "name", None)):
        return True
    try:
        from plugins.web.keyless_mcp import _KEYLESS_RING, use_keyless
        ...
        logger.debug("search rescue gate check failed: %s", exc)
        return False
